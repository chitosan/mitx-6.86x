...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    Args:
        `feature_vector` - numpy array describing the given data point 1xN.
        `label` - float, the correct classification of the data point.
        `theta` - numpy array describing the linear classifier 1xN.
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    Args:
        `feature_matrix` - numpy matrix describing the given data. Each row
            represents a single data point MxN.
        `labels` - numpy array where the kth element of the array is the
            correct classification of the kth row of the feature matrix 1xN.
        `theta` - numpy array describing the linear classifier 1xM.
...
def perceptron_single_step_update(
    ...
    if (label*np.dot(theta, feature_vector) + theta_0) <= 1e-8:
...
def perceptron(feature_matrix, labels, T):
    ...
    raise NotImplementedError
        for t in range(T):
        for i in get_order(nsamples): #get_order(nsamples)
            z = labels * (np.dot(theta*feature_vector)+theta_0)
            if z<=0:
                theta = theta + labels*feature_vector
                theta_0 = theta_0 + labels
            raise NotImplementedError
    """
