...
#print('Linear Regression test_error =', run_linear_regression_on_MNIST(lambda_factor=1.0))
...
#print('SVM one vs. rest test_error:', run_svm_one_vs_rest_on_MNIST(1.0))
...
def run_softmax_on_MNIST(temp_parameter=1):
    ...
    print(" SOFTMAX ")
    train_x, train_y, test_x, test_y = get_MNIST_data()
    theta, cost_function_history = \
        softmax_regression(train_x, train_y, temp_parameter,
             alpha=0.3, lambda_factor=1.0e-4, k=10, num_iterations=150)
    plot_cost_function_over_time(cost_function_history)
    test_error = compute_test_error(test_x, test_y, theta, temp_parameter)
    # Save the model parameters theta obtained from calling softmax_regression to disk.
    write_pickle_data(theta, "./theta.tar.gz")
    return test_error


#print('softmax test_error=', run_softmax_on_MNIST(temp_parameter=1.0))

# TODO: Find the error rate for temp_parameter = [0.5, 1.0, 2.0]
...
def run_softmax_on_MNIST_mod3(temp_parameter=1):
    ...
    theta, cost_function_history = \
        softmax_regression(train_x, train_y_mod3, temp_parameter,
            alpha= 0.3, lambda_factor = 1.0e-4, k = 10, num_iterations = 150)
    # plot_cost_function_over_time(cost_function_history)
...
#print("softmax on MNIST mod3 error: ",run_softmax_on_MNIST_mod3(temp_parameter=1.0))
...
theta, cost_function_history = softmax_regression(
    train_pca, train_y,
    temp_parameter=0.5, alpha=0.3, lambda_factor=1.0e-4, k=10, num_iterations=150)
plot_cost_function_over_time(cost_function_history)
...
# Step 1: Calculate the 10-dimensional PCA representation
...
# Perform PCA to reduce the dimensionality to 10
