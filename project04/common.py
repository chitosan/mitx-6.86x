...
def bic(X: np.ndarray, mixture: GaussianMixture,
    ...
    # number of parameters = (number of means + number of covariances + number of mixing probabilities) * k
    num_params = (model.means_.size + model.covariances_.size + (k - 1)) * k
    ...
    for i in range(len(mixture)):
        if i == 0:
            p = p + mixture[i].shape[0] * mixture[i].shape[1]
        else:
            p = p + mixture[i].shape[0]
    p = p - 1
    b = log_likelihood - (p * np.log(n)) / 2.0
