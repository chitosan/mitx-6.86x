...
for seed in range(5):
    ...
    print(" Total cost dictionary update:", total_cost)
...
seeds = [0,1,2,3,4]
mixture = []
cost = np.zeros([len(seeds),len(clusters)])
bic = np.zeros([len(seeds),len(clusters)])
for i in clusters:  # appending empty objects
    mixture.append(common.GaussianMixture(0,0,0))

# Randomly Initialize Clusters
for seed in seeds:
    for K in clusters:
        mixture[K], post = common.init(X,K+1,seed)
        mixture[K], post, cost[seed][K] = kmeans.run(X, mixture[K], post)
        #bic[K] = common.bic(X,mixture[K],cost[seed][K])
        #if seed==4:
        #    common.plot(X, mixture[K], post, "K-Means")
print("Cost: ")
print(cost)
print("BIC")
print(bic)
print(np.min(cost,axis=0))


mixture[0], post = kmeans.init(X, 1, 0)
for seed in seeds:
    for K in clusters:
        #mixture[K], post = naive_em.init(X, K + 1, seed)
        mixture[K], post, cost[seed][K] = naive_em.run(X, mixture[K], post)
        #bic[K] = common.bic(X, mixture[K], cost[seed][K])
        #if seed==2:
        #    common.plot(X, mixture[K], post, "naive_em")
